...
1. `pypdfium2.PdfDocument(文件路径)` 打开 PDF（别名 `pdfium`）
2. 逐页（`for page in pdf`）调用 `page.render(scale=2)`（scale=2 保证识别质量）
3. `bitmap.to_pil()` 得到 PIL 图片，送多模态模型识别
4. 处理完毕 `pdf.close()`
...
不同职教任务对错误容忍度不同，自由度应匹配任务后果：

| 任务类型 | 自由度 | 原因 | 分析要求 |
|---------|--------|------|---------|
| **安全规程错误** | 低 | ⚠️ 高风险，错误可能导致人身伤害 | 必须明确指出违反的具体安全条款，给出严格的纠正步骤 |
| **设备操作流程** | 低 | 顺序错误可能导致设备损坏或产品报废 | 必须指出具体哪一步顺序错误，给出标准操作顺序 |
| **工艺参数设置** | 低 | 参数错误直接影响产品质量 | 必须指出错误参数值，给出正确参数范围和依据 |
| **编程/代码逻辑** | 中 | 语法错误可调试，但逻辑错误影响功能 | 指出错误类型（语法/逻辑），提供调试思路而非直接给答案 |
| **工艺优化建议** | 高 | 多种方案可达到相同质量标准 | 提供多种优化思路，让教师/学生根据实际条件选择 |
| **职业情境分析** | 高 | 岗位场景多变，需灵活适配 | 提供分析框架，鼓励结合具体岗位场景思考 |

**自由度应用原则**：
- 低自由度任务 → 给具体纠正步骤，不提供"其他方案"
- 高自由度任务 → 提供多种思路和决策框架，不指定唯一答案
